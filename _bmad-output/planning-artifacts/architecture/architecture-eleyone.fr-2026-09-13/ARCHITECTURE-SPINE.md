...
- **Rule:**
  - **Fondement légal (révisé le 23/09/2026).** L'obligation d'identification est portée par l'**article 1-1 de la loi n° 2004-575 du 21 juin 2004** (LCEN) et la sanction par son **article 1-2**, depuis la loi n° 2024-449 du 21 mai 2024 (SREN), en vigueur au 23/05/2024. **L'article 6 III n'existe plus** : toute référence à lui, y compris sur la fiche service-public.fr datée du 31/07/2023, est périmée. S'y ajoute l'**article 19** de la même loi, qui impose l'adresse de courrier électronique et des coordonnées téléphoniques à qui exerce une activité de commerce électronique au sens large de l'article 14 — ce qu'est un site vitrine professionnel, même sans vente en ligne.
  - Les valeurs sont lues **uniquement** dans `_partials/legal-value.html`, par `os.Getenv`, sous **huit** noms définitifs, **une variable par donnée** (décidé le 13/09/2026 pour sept d'entre elles, révisé le 23/09/2026 puis le 24/09/2026 : arbitrages d'Arnaud après recherche des obligations réelles) : `HUGO_LEGAL_PUBLISHER_NAME`, `HUGO_LEGAL_PUBLISHER_ADDRESS`, `HUGO_LEGAL_PUBLISHER_EMAIL`, `HUGO_LEGAL_PUBLISHER_PHONE`, `HUGO_LEGAL_PUBLISHER_REGISTRATION`, `HUGO_LEGAL_HOST_NAME`, `HUGO_LEGAL_HOST_ADDRESS`, `HUGO_LEGAL_HOST_EMAIL`.
    - Les deux `*_CONTACT` d'origine sont **remplacées**, non complétées : une valeur composée « courriel et téléphone » interdisait de traiter chaque donnée pour ce qu'elle est — un `mailto:` pour l'une, un `tel:` pour l'autre — et empêchait un contrôle de dire laquelle manque. Le compte passe de sept à huit et non à dix.
    - **Le téléphone de l'éditeur est obligatoire** (art. 1-1 I 1° : « leurs nom, prénoms, domicile et numéro de téléphone »), et **celui de l'hébergeur aussi** (art. 1-1 I 4°). Le numéro de l'éditeur publié est un **numéro dédié à l'activité**, distinct du numéro personnel (arbitrage d'Arnaud, 23/09/2026) : le numéro personnel reste une donnée privée et **reste dans la liste des motifs interdits**, qui continue donc de le refuser partout.
    - **Le téléphone de l'hébergeur : une obligation inexécutable, et un écart assumé (24/09/2026).** L'obligation de l'article 1-1 I 4° ci-dessus **n'est pas amendée** : elle demande bien un numéro. Le constat est factuel et daté : l'hébergeur retenu n'en publie aucun — ses pages de contact, de mentions, de conditions, d'assistance et de présentation ont toutes été chargées et cherchées — et **il l'écrit lui-même** sur sa page d'aide consacrée au sujet, où il indique ne pas offrir de support téléphonique. La seule donnée de cette forme qu'il déclare vit dans un registre public d'attribution d'adresses IP ; rien n'en fait un numéro joignable au sens de la LCEN.
      - La variable `HUGO_LEGAL_HOST_PHONE` est donc **remplacée** par `HUGO_LEGAL_HOST_EMAIL`, dont la valeur est l'adresse que l'hébergeur désigne pour les signalements et pour les autorités de l'Union européenne. Le compte reste à huit.
      - **Ce courriel n'est pas réputé satisfaire l'article 1-1 I 4°.** Le texte demande un numéro ; publier une adresse à la place ne le requalifie pas, et aucune source ne permet de l'affirmer. C'est un **écart, assumé et écrit ici**, faute de pouvoir faire mieux : le site publie ce qu'il peut vérifier plutôt qu'un numéro inventé.
      - Le risque écarté est précis. Un numéro circule sur des mentions légales françaises de sites hébergés là ; il n'a de source chez l'hébergeur nulle part et paraît recopié de proche en proche depuis une invention. Le reprendre aurait publié un faux dans la seule page du site dont l'objet est d'être exacte. **Aucun numéro non sourcé n'entre dans le dépôt ni dans un fichier de valeurs.**
      - Les deux pages des mentions légales **disent l'absence**, dans le corps Markdown et non dans un gabarit (AD-3), en attribuant le fait à l'hébergeur plutôt qu'en l'énonçant au nom du site : le lecteur sait qui l'affirme et peut le vérifier.
      - **À réexaminer si l'hébergeur change, ou s'il se met à publier un numéro.** Le remplacement inverse est alors une modification de cet AD, comme toute variable légale.
    - **Article 1-1 I 5° (stockage des données), créé par la loi SREN** : il faut déclarer qui stocke les données traitées quand ce n'est pas l'hébergeur des pages. Le site est statique, sans formulaire, sans analytique et sans JavaScript (AD-3) : **aucun tiers ne stocke de donnée traitée, donc rien à déclarer et aucune variable**. Réexaminer si une story ajoute un formulaire, une mesure d'audience ou un service tiers. Aucune source officielle traitant explicitement le cas du site statique n'a été trouvée ; la conclusion se déduit du texte.
...
- **Rule:**
  ...
  - **Identifiants de poste** (décidé le 13/09/2026, D-8) : `position-<société en kebab-case>` quand la société n'apparaît qu'une fois dans le parcours ; `position-<société>-<année de début>` quand elle apparaît plusieurs fois. Exception : `position-earlier-career` désigne un regroupement (« Parcours antérieur », 2008–2014), pas une société. La liste est figée ici, avant la rédaction des cas 01, 05 et 06 et avant la story 10.2 ; `docs/format-cas.md` y renvoie. Liste exacte décidée par Arnaud le 13/09/2026 (compléments de la proposition de changement) :
    ...
    La liste est complète : aucun autre poste n'est créé sans modifier cet AD, et aucun identifiant du tableau n'est renommé. La mission chez CGI n'est pas un poste : c'est une ligne de détail de `position-earlier-career`. **Le sens a été corrigé le 24/09/2026** (story 10.2, précision d'Arnaud) : il était en prestation **chez CGI**, par l'ESN **FHM Solutions**, sur un projet CGI **pour le compte d'April assurance**. Cet AD écrivait l'inverse — « chez April pour le compte de CGI » — et datait la mission de 2013–2014, dates qu'aucune source ne porte : le CV date l'employeur, « FHM Solutions (2011-2014) », et pas la mission. Le contenu reprend le CV sans ajouter de date.
  - **Front matter d'un poste** (clés en anglais) :
    - `company` (non traduit) ; `role` (dans la langue du fichier) ;
    - `label` (facultatif, **dans la langue du fichier**) : le nom affiché quand le poste n'est pas une société. Ajoutée le 24/09/2026 (story 10.2, arbitrage d'Arnaud). La proposition de changement du 13/09/2026 laissait le point ouvert en toutes lettres — « la clé `company` d'un poste n'est pas traduite (C3), alors que le regroupement `position-earlier-career` porte un libellé à traduire ; la clé qui porte ce libellé reste à fixer à la story 10.2 » — et le voici fixé. `position.html` affiche `label` à la place de `company` quand elle est présente ; `company` devient alors facultative, et **l'un des deux au moins est requis**. `label` n'entre **pas** dans la liste des clés que C3 compare entre les langues, précisément parce qu'elle se traduit : « Parcours antérieur » en FR, « Earlier career » en EN. `company` y reste, pour que « Mister Auto » ne devienne jamais « MisterAuto » d'un seul côté. Les trois autres options ont été écartées devant Arnaud : laisser le libellé français sur l'accueil anglais, qui vise les recruteurs internationaux ; sortir `company` de C3, qui détruirait une garde générale pour un cas particulier ; éclater le regroupement en sociétés réelles, qui contredirait la liste figée ci-dessus ;
  ...
  - **Cas 06** : rattaché au poste `position-april-technologies-2017` (April Technologies, 2017, en prestation Modis ; PRD §11.1, question 15) ; le lien avec la mission chez CGI pour le compte d'April assurance (ligne de détail de `position-earlier-career`) peut figurer dans le corps du poste, sans clé dédiée.
...
- **Rule:**
  ...
  - Le partial isole les segments `<pre>…</pre>`, `<code>…</code>` et les balises (donc les attributs et les URL) avant `replaceRE`, puis les réassemble. Les blocs de code sont **comptés**, pas marqués : un `code` dans un `pre` ne rouvre pas la composition en se fermant.
  - **Un appel par gabarit principal**, pas un appel par titre ou par libellé : Hugo ne sait pas capturer la sortie d'un `block`, donc chaque `define "main"` écrit son corps dans un partial en ligne et le fait traverser `typo-fr.html`. `scripts/tests/test-typo.sh` refuse un gabarit qui définirait `main` sans passer par là — c'est ce test qui remplace la garantie qu'un point d'application unique aurait donnée (story 6.3).
  - **Le `<title>` est composé à part**, dans `baseof.html` : il vit hors du corps et aucun gabarit ne l'atteint. C24 l'a montré sur le cas 01, dont l'onglet affichait « au produit : une calculette » avec une espace ordinaire. Un `<title>` n'admet aucun balisage, donc le texte y passe directement par les substitutions. Il remplace l'espace ordinaire placée devant `;`, `!`, `?` et `:`, et à l'intérieur des guillemets « », par l'espace insécable que prescrit `DESIGN.md` (« Typographie française et anglaise ») ; il n'insère pas d'espace là où l'auteur n'en a pas mis. Aucune dépendance n'est ajoutée. Validé sur le pilote dans la story.
...
| Secrets et variables CI | Gitea uniquement. Secrets : `ANTHROPIC_API_KEY`, `DEPLOY_SSH_KEY`, `DEPLOY_HOST`, `DEPLOY_KNOWN_HOSTS`, `PRIVATE_PATTERNS`. Valeurs légales (lues seulement par le workflow `release`) : les huit `HUGO_LEGAL_*` d'AD-9. GitHub : aucun. Les variables `GITEA_*` du poste de développement n'ont pas d'équivalent en CI : Gitea interdit ce préfixe pour un secret, et les jobs utilisent le jeton automatique `GITEA_TOKEN`. |
...
10. Variables légales définitives : les sept `HUGO_LEGAL_*` ; directeur de la publication = éditeur ; pas de TVA intracommunautaire (AD-9). — *Révisé le 23/09/2026 : huit variables, une par donnée, les deux `*_CONTACT` étant remplacées par `PUBLISHER_EMAIL`, `PUBLISHER_PHONE` et `HOST_PHONE`. Révisé de nouveau le 24/09/2026 : `HOST_PHONE` est remplacée par `HOST_EMAIL`, l'hébergeur retenu ne publiant aucun numéro de téléphone — écart assumé, détaillé dans AD-9. Le relevé ci-dessus reste ce qui a été décidé ce jour-là ; AD-9 fait foi pour l'état courant.*
