...
- **Rule:**
  ...
  - Les valeurs sont lues **uniquement** dans `_partials/legal-value.html`, par `os.Getenv`, sous **huit** noms définitifs, **une variable par donnée** (décidé le 13/09/2026 pour sept d'entre elles, révisé le 23/09/2026 : arbitrage d'Arnaud après recherche des obligations réelles) : `HUGO_LEGAL_PUBLISHER_NAME`, `HUGO_LEGAL_PUBLISHER_ADDRESS`, `HUGO_LEGAL_PUBLISHER_EMAIL`, `HUGO_LEGAL_PUBLISHER_PHONE`, `HUGO_LEGAL_PUBLISHER_REGISTRATION`, `HUGO_LEGAL_HOST_NAME`, `HUGO_LEGAL_HOST_ADDRESS`, `HUGO_LEGAL_HOST_PHONE`.
    - Les deux `*_CONTACT` d'origine sont **remplacées**, non complétées : une valeur composée « courriel et téléphone » interdisait de traiter chaque donnée pour ce qu'elle est — un `mailto:` pour l'une, un `tel:` pour l'autre — et empêchait un contrôle de dire laquelle manque. Le compte passe de sept à huit et non à dix : la loi n'attend de l'hébergeur qu'un téléphone (art. 1-1 I 4°), pas un courriel.
    - **Le téléphone de l'éditeur est obligatoire** (art. 1-1 I 1° : « leurs nom, prénoms, domicile et numéro de téléphone »), et **celui de l'hébergeur aussi** (art. 1-1 I 4°). Le numéro publié est un **numéro dédié à l'activité**, distinct du numéro personnel (arbitrage d'Arnaud, 23/09/2026) : le numéro personnel reste une donnée privée et **reste dans la liste des motifs interdits**, qui continue donc de le refuser partout.
