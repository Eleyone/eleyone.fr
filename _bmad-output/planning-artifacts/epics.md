...
**Étant donné** `scripts/publish-case.sh <translationKey>` **sans** `--relu`
**Quand** le script s'exécute
**Alors** il lance tous les contrôles, affiche les fichiers qu'il passerait hors brouillon et les clés qu'il ajouterait, et **ne modifie rien** : la relecture humaine qu'exige le format est un acte explicite, et `--relu` en est la trace (décidé par Arnaud le 21/09/2026).

**Étant donné** un cas en brouillon
**Quand** un contrôle échoue (format, parité, stack, garde-fou), qu'un marqueur `[TODO` reste dans **les fichiers du cas ou de son groupe**, ou que le **poste** du cas est encore en brouillon
**Alors** le script s'arrête sans rien modifier et nomme l'écart et le fichier. Il ne publie jamais le poste à la place de la story qui en a la charge.

**Étant donné** un cas qui passe tout, et `--relu`
**Quand** le script s'exécute
**Alors** il crée la branche `feat/publish-case-<translationKey>`, remplace la valeur de `draft:` par `false` dans le **front matter** des deux fichiers du cas — et, si c'est le **premier cas publié de son groupe**, dans les deux fichiers `_index` du groupe, que D-3 veut dans la même PR (AD-4, décidé par Arnaud le 21/09/2026) —, ajoute à `ci/release-pages.txt` le `translationKey` du cas et, pour un cas groupé, `group-<group>`, chacun seulement s'il en est absent (D-5), le tout dans un seul commit
**Et** il relance `scripts/check.sh`, pousse la branche, puis ouvre la PR par `create-pull-request` avec un corps qui nomme les fichiers publiés et les clés ajoutées.

**Étant donné** les fichiers du cas
**Quand** le script les cherche
**Alors** il les trouve par le **manifeste** du rendu de travail, jamais par un chemin deviné : le manifeste porte le rôle, la clé de traduction, le brouillon, le groupe, le poste et la présence de `[TODO` (AD-10).

- [ ] La décision — quoi publier, quoi ajouter, quoi refuser — vit dans `scripts/lib/publish-case.sh` et s'éprouve sur des manifestes écrits à la main ; le script fait le build, les contrôles, git et la forge.
- [ ] Seule la ligne `draft:` du front matter est réécrite, et le script vérifie qu'il a changé une ligne et une seule par fichier.
- [ ] SKILL.md et `docs/procedures/publish-case.md` suivent le principe des trois niveaux, et la description du skill dit quand l'employer.
...
**Couvre :** NFR-3, NFR-5, NFR-12, FR-19, FR-21 · AD-13, AD-15 (**conteneur seulement** : le reverse proxy se configure à la première mise en ligne, epic 11)
**Dépendances :** 4.1
**Bloquée par :** —
**Prérequis de contenu :** —
**Opération manuelle (Arnaud) :** non
...
**Alors** la réponse porte `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `Cache-Control: no-cache` et la politique `Content-Security-Policy: default-src 'none'; style-src 'self'; img-src 'self'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'`, sans `Set-Cookie`.

**Étant donné** un fichier empreinté (`\.[0-9a-f]{64}\.(css|svg|webp)$`)
**Quand** on lance `curl -I`
**Alors** la réponse porte `Cache-Control: public, max-age=31536000, immutable`, les deux en-têtes de sécurité, aucun `Set-Cookie`, et **aucune** `Content-Security-Policy` — les SVG de D2 portent des `<style>` et des polices embarquées.

**Étant donné** un fichier ni HTML, ni PDF, ni empreinté — la photo en est le cas typique
**Quand** on lance `curl -I`
**Alors** la réponse porte `Cache-Control: no-cache` (décidé par Arnaud le 21/09/2026) : le navigateur revérifie, ne retransfère rien si rien n'a changé, et une photo remplacée est vue tout de suite.

**Étant donné** un PDF
**Quand** on lance `curl -I`
**Alors** la réponse porte `Cache-Control: no-cache` : un CV remplacé ne reste pas en cache.

**Étant donné** une URL absente sous `/` puis sous `/en/`
**Quand** on la demande
**Alors** le statut est **`404`** — servir la bonne page en `200` ferait indexer les erreurs —, le corps est celui de `/404.html` puis de `/en/404.html`, et les en-têtes de sécurité sont là (`always`).

**Étant donné** une requête `Accept-Encoding: gzip` sur du HTML, du CSS, du JSON et du XML
**Quand** on lance `curl -I`
**Alors** la réponse porte `Content-Encoding: gzip` et `Vary: Accept-Encoding`.

**Étant donné** une URL de dossier sans barre oblique finale
**Quand** on la demande
**Alors** la redirection porte un `Location` **relatif** (`absolute_redirect off`) : derrière un proxy, une adresse absolue renverrait vers le port du conteneur.

**Étant donné** une requête avec chaîne de requête, user-agent et referer
**Quand** on lit le journal du conteneur
**Alors** la ligne ne contient que date, méthode, `$uri`, statut et taille — ni adresse IP, ni chaîne de requête, ni user-agent, ni referer.

- [ ] `server_tokens off; absolute_redirect off; log_not_found off;` ; `error_log /dev/stderr crit;` ; aucun module `realip`.
- [ ] Les en-têtes sont déclarés au niveau `server` avec `always`, et `add_header_inherit merge;` — sans quoi un `add_header` dans un `location` effacerait ceux du `server`. La directive existe dans l'image servie (vérifié le 21/09/2026, nginx 1.30.4).
- [ ] La CSP n'est envoyée que sur le HTML, par `map $sent_http_content_type $csp`.
- [ ] Les essais tournent contre un **vrai conteneur** ; les cas hors ligne vérifient la configuration elle-même, la suite restant sans réseau ni démon (story 0.9).
