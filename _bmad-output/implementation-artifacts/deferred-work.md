...
- source_spec: `_bmad-output/implementation-artifacts/11-2-forbidden-patterns-on-production-output.md`
  summary: C22 et C23 ne rapprochent pas deux **caractères** différents, seulement deux écritures d'un même caractère. Le décodage couvre désormais les entités et les séquences `\uXXXX`, `&rsquo;` comprise (story 11.2) ; il ne peut rien contre un motif écrit avec l'apostrophe droite `'` là où la page porte l'apostrophe typographique `’`, ni contre une espace ordinaire là où la page porte U+00A0 ou U+202F (que C24 pose devant la ponctuation française). La liste des motifs est une liste littérale, comme pour les accents, et `grep -i` ne replie que la casse. À reprendre en repliant les deux côtés — texte **et** motifs, sans quoi le repli déplace le trou — si un motif de la vraie liste porte une apostrophe ou une espace fine ; le faire suppose de transformer les fichiers temporaires de motifs, ce que C21 et le garde-fou ne font pas, donc de décider si la règle vaut pour les trois.
  evidence: Mesuré sur le vrai build le 25/09/2026 : `&rsquo;` est la **seule** entité que la sortie de production émet, 236 fois, et un motif portant `’` n'était vu dans aucune page avant la story 11.2. La sortie ne porte en revanche aucune entité d'espace : U+00A0 et U+202F y sont écrits en clair, donc un motif à espace ordinaire ne les croise pas.

- source_spec: `_bmad-output/implementation-artifacts/11-2-forbidden-patterns-on-production-output.md`
  summary: C12 garde sa propre résolution d'une URL en chemin de fichier. La règle « `/cas/chiliz/` désigne `cas/chiliz/index.html` » vit désormais dans `checks_page_de_url` (`scripts/checks/lib.sh`), employée par C15 et par C22 ; `page_visee` (`scripts/checks/links.sh:48`) en porte encore la sienne, enveloppée dans une résolution plus large qui traite aussi les liens relatifs et les fragments. Trois exemplaires étaient la configuration exacte du constat A1 de la rétrospective de l'epic 3, où le même correctif avait dû être appliqué trois fois ; il en reste deux. À reprendre en faisant appeler `checks_page_de_url` par `page_visee` pour ses deux dernières lignes, dans une story qui rouvre déjà C12 — la boucle des liens est chaude, et une substitution de commande par lien s'y paie.
  evidence: Story 11.2 (25/09/2026) : C22 avait besoin de la même résolution, et l'extraction a été faite pour C15 et C22 seulement, afin de ne pas rouvrir C12 hors de son périmètre. `scripts/tests/test-links.sh` et `scripts/tests/test-release-pages.sh` passent sans modification.

- source_spec: `_bmad-output/implementation-artifacts/11-2-forbidden-patterns-on-production-output.md`
  summary: C23 code en dur les chemins des deux pages des mentions légales. `readonly pages_legales=(mentions-legales/index.html en/legal-notice/index.html)` (`scripts/checks/legal-address.sh:46`) vient des permaliens décidés le 13/09/2026 ; C22 les trouve désormais par le manifeste, en lisant l'`url` de l'entrée de `translationKey` `legal-notice`, clé livrée par la story 11.1. Deux contrôles répondent donc différemment à la même question, et un changement de `slug` dans `content/legal-notice.fr.md` rendrait C23 aveugle en silence : il ne verrait plus aucune page légale, admettrait l'adresse nulle part et la signalerait partout — bruyant, donc pas un faux négatif, mais faux quand même. À reprendre en faisant lire le manifeste à C23, dans une story qui rouvre déjà ce contrôle ; la divergence est décidée, pas subie (ligne A5 du triage de la revue de spec de la story 11.2).
  evidence: Constat bloquant de la revue de spec de la story 11.2 (25/09/2026, `gemini-3.1-pro-high`), retenu pour C22 et explicitement non étendu à C23 pour ne pas élargir la story. `scripts/checks/output-patterns.sh` lit le manifeste ; `scripts/checks/legal-address.sh:46` garde son tableau.

- source_spec: `_bmad-output/implementation-artifacts/11-2-forbidden-patterns-on-production-output.md`
  summary: C22 et C23 parcourent `public/` par `checks_find … -printf '%P\n'` puis `while IFS= read -r`, si bien qu'un fichier dont le nom contient un saut de ligne est lu en deux morceaux. Les deux **refusent bruyamment** — code 2, « rien n'est affirmé » —, donc aucun n'est un faux négatif, à la différence du constat B5 de la rétrospective de l'epic 5, où un nom non-ASCII échappait à C20 sans un mot. Un tel nom ne peut pas venir d'un slug de Hugo, mais un fichier de `static/` est copié tel quel. À reprendre en passant les deux contrôles à `-print0`, `sort -z` et `read -r -d ''` **dans la même story** : corriger l'un sans l'autre est la faute du point 19.
  evidence: Constat non bloquant de la revue du code de la PR n° 117 (25/09/2026, `gemini-3.1-pro-high`), reproduit par l'orchestrateur sur une copie de `public/` portant un fichier au nom coupé — `output-patterns.sh` rend 2, `legal-address.sh` écrit le même message ; C15 n'est pas concerné, il ne parcourt pas les sorties non HTML.
