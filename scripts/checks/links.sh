...
attributs() { # $1 = fichier, $2 = requête, $3 = nom de l'attribut
  xpath "$1" "$2" | { grep -oE "$3=\"[^\"]*\"" || true; } | sed -E "s/^$3=\"(.*)\"$/\1/"
}
...
mapfile -t pages < <(cd "$public" && find . -type f -name '*.html' | sed 's#^\./##' | LC_ALL=C sort)
...
for page in "${pages[@]}"; do
  ...
  while IFS= read -r href; do
    ...
    if [[ -n $fragment ]]; then
      ...
      grep -qxF "$fragment" <<< "$identifiants" \
        || signaler "$page" "C12 : ancre « #$fragment » absente de $visee"
    fi
  done < <(attributs "$public/$page" '//a[@href]/@href' href)
...
liens_cv=$({ grep -rlE 'href="?/assets/cv/' "$public" --include='*.html' || true; } | wc -l)
...
liens_depot=0
[[ -z $source_url ]] || liens_depot=$({ grep -rlF "$source_url" "$public" --include='*.html' || true; } | wc -l)
