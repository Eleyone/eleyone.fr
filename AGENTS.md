...
4. **Merge gates.** Nothing is merged without a review report on the head SHA with a non-blocking verdict, the public/private guard passing, green CI once CI exists, and a consistent sprint status (statuses only in v1). The release PR `dev` → `main` meets the review gate when every commit in `main..dev` is the squash of a PR merged by `verify-and-merge-pr`, and `release` merges only with an explicit `--merge` run by Arnaud. The agent prepares the PR and waits; merging is a human decision. **Sprint status lives in the story's own PR:** the story's line in `sprint-status.yaml` moves to `in-progress` in the branch's first commit, to `review` in a commit made before the LLM review, and to `done` in a commit made after a passing review, just before merge. The review gate therefore accepts a report on the parent of the head SHA when the head commit is that single status commit: it changes only `sprint-status.yaml`, and in it only the story's line from `review` to `done`, `last_updated`, and the epic's line when the story closes its epic, checked line by line. Any other change after the review requires a new review.
