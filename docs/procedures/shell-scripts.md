...
- La logique commune vit dans `scripts/lib/` : `shell.sh` (enveloppes des outils lancés en boucle), `gitea.sh` (environnement, API de la forge, fichier de motifs), `sprint.sh` (lecture du suivi de sprint, sans `jq`), `merge-gates.sh` (décisions des verrous de fusion), `publish-case.sh` (décision de publication).
- **Une parade s'écrit une fois.** Dupliquer une garde correcte est un défaut au même titre que l'oublier : la même lecture du code de `grep` avait fini en quatre exemplaires, dont le dernier est né le jour où l'avant-dernier a été écrit pour cette raison exacte (rétrospective de l'epic 3). Avant d'écrire une enveloppe, chercher si elle existe dans `scripts/lib/shell.sh`.
...
- **Chercher dans un fichier** : `shell_grep_into <variable> <arguments de grep>`, l'enveloppe commune du dépôt (`scripts/lib/shell.sh`). Elle distingue « rien trouvé » d'une erreur de lecture — deux cas que `grep` rend par 1 et par 2 ou plus —, et remplit une variable de l'appelant plutôt que d'écrire sur la sortie standard : appelée dans `$(…)`, une fonction ne peut pas arrêter le cas, son `exit` ne quittant que le sous-shell. **Elle rend toujours 0**, y compris sans correspondance : l'absence se lit dans la variable, vide. C'est ce qui permet de l'appeler nue sous `set -e` sans `|| true` — le `|| true` est précisément ce que ces enveloppes existent pour supprimer, puisqu'il avalerait aussi l'erreur de lecture. Qui a besoin de distinguer les deux appelle `shell_grep_status`, qui rend le code de `grep`.
...
- **La suite ne garde pas que des scripts.** `scripts/tests/test-docs-headings.sh` refuse un titre de niveau 1 ou 2 répété dans un document suivi du dépôt. C'est le seul verrou qui lise un document *dans son ensemble* : la revue lit un diff, `check.sh` lit le site rendu, `sprint-consistency` lit `sprint-status.yaml`. Il est né de la corruption d'`epics.md`, qui a vécu six stories sans être vue (rétrospective de l'epic 4). À partir du niveau 3, une répétition est légitime — un fichier de story porte une rubrique par lentille et par revue subie.
- **La feuille de style y est tenue aussi.** `scripts/tests/test-design-tokens.sh` confronte `assets/css/main.css` au bloc de tokens en tête de `DESIGN.md` : une couleur inventée d'un chiffre y échoue, et aucune police ne peut y être chargée. Une valeur de design ne se relit pas — `#1c6b46` ressemble à `#1c6b45`.
...
| cas de test qui suppose son environnement | trois formes déjà rencontrées : un rendu de travail présent dans le dépôt (vrai sur le poste, faux en CI, où la suite tourne avant tout build) ; `case_…_hors_image` comptait sur l'absence d'`apk` : vrai sur le poste, faux dans `CHECK_IMAGE`, où la suite tourne **dans** le job et où le cas relançait le job entier ; de même, une variable que le cas veut absente (`HOST_UID`) lui est léguée par le conteneur | réduire le `PATH` du cas à ce qu'il lui faut, retirer nommément les variables (`env -u`) : un cas doit rendre le même verdict des deux côtés | story 3.12 |
