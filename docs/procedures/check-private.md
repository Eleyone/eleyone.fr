...
- **Chemins interdits**, comparés sans tenir compte de la casse : `docs/private/`, `docs/context/`, `.env` à toute profondeur — seul ou suffixé (`.env.production`, `.env.local`), alors que `.environment.md`, `env.example` et `docs/env.md` restent admis —, `assets/cv/*.pdf` tant que le hook serveur ne sait pas lire les PDF (AD-21), et les extensions d'images (`.jpg`, `.png`, `.webp`…) tant que C20 ne vit pas dans le hook (AD-19, story 5.4). Le SVG reste admis : les schémas D2 sont commités et lisibles comme du texte. **Deux exceptions nommées** : `.env.example` (commité par conception, sans aucune valeur) et `design/<branche>/screenshots/` (captures déjà publiées des branches de design). Nommer `docs/private/` dans un fichier est permis ; y placer un fichier ne l'est pas.
- **Chemins confrontés aux motifs** : un fichier ou un dossier nommé d'après un client fuit autant que son contenu. Le chemin fautif n'est **pas affiché** — l'afficher reviendrait à afficher le motif ; l'alerte donne le commit et le numéro de ligne du motif.
- **Messages de commit** : en modes `history` et `pre-receive`, le message de chaque commit examiné est confronté aux motifs, et n'est jamais affiché. En mode `staged`, le message n'existe pas encore : il échappe au pre-commit et sera refusé au push.
