...
| C15 | `scripts/checks/release-pages.sh` | au niveau `release` seulement : une clé de `ci/release-pages.txt` inconnue du manifeste, une page attendue absente du build de production en FR ou en EN, la section d'un cas groupé absente de la page de son groupe, une page de groupe qui ne porte aucune `<section class="case-section">`, un `checks.json` publié, une chaîne `VALEUR-FACTICE`, `noindex` ou `draft-marker` dans une page. La liste est **cumulative** et ne vaut qu'à ce commit (D-5) : la complétude du socle n'est pas vérifiée ici |
| C22 | `scripts/checks/output-patterns.sh` | au niveau `release` seulement : toute occurrence d'un motif de la liste (`PRIVATE_PATTERNS_FILE`, à défaut `docs/private/forbidden-patterns.txt`) dans un fichier de `public/`, images (C20) et CV publiés (C21) exceptés. Sur les deux pages des mentions légales, les valeurs `HUGO_LEGAL_*` injectées sont **expurgées** du texte avant la confrontation : une occurrence contenue dans l'une d'elles est légitime, la même chaîne écrite ailleurs sur la page ne l'est pas. Chaque fichier est décodé et normalisé avant d'être lu. Liste des motifs absente ou vide, valeurs légales absentes, page légale introuvable dans le manifeste : **anomalie**, jamais un succès. Aucun message n'affiche le motif ni l'extrait |
