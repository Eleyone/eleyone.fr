...
- **Niveau** : `CHECK_LEVEL` vaut `standard`, ou `release` avec `--release`. Un contrôle de mise en ligne ne juge rien hors de `release` — `dev` porte des cas en brouillon et les valeurs légales factices, et il y échouerait à chaque PR —, mais il **dit** qu'il est sauté avant de rendre `0` : un `exit 0` muet cacherait un nom de variable mal écrit. C15 (`scripts/checks/release-pages.sh`) est le premier de cette famille.
